...
if [ -z "$2" ]; then
    echo "No node name was specified"
    exit 1
fi
...
# Display header
echo "============================================================="
echo "Creating Scaleup ISO for cluster ${CLUSTER_NAME} for Node:"
echo " ${NODE_NAME}"
echo "============================================================="
...
ansible-playbook -e node_name=${NODE_NAME} -e "@${SITE_CONFIG_DIR}/${1}/cluster.yml" -e pull_secret_path="$(realpath ./${SITE_CONFIG_DIR}/${1}/ocp-install-pull-secret.json)" -e "@${SITE_CONFIG_DIR}/${1}/nodes.yml" playbooks/create-scaleup-iso.yml
